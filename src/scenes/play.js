class Play extends Phaser.Scene {
  ...
  create() {
    ...
    // this.coalend.depth(10);
    // this.niceend.depth(10);
    ...
    this.nice = this.add.tileSprite(475, 175, 256, 256, 'nice').setOrigin(0, 0).setScale(0.6);
    this.naughty = this.add.tileSprite(25, 175, 512, 256, 'naughty').setOrigin(0, 0).setScale(0.6);

    this.karmabar = this.add.tileSprite(200, 50, 512,70, 'karmabar').setOrigin(0,0).setScale(0.8);

    this.snowflake = this.add.tileSprite(375, 60,64,64, 'snowflake').setOrigin(0,0).setScale(0.7);


    this.matter.add.mouseSpring({ length: 1, stiffness: 1 }); //this is what lets you click on stuff

    this.letter = this.matter.add.image(100, 150, "letter").setBounce(0.9).setScale(0.1);
    ...
    this.matter.world.on("collisionstart", (event) => {
      ...
      this.nice.setVisible(false);
      this.naughty.setVisible(false);
      if (this.letter.x < 0) {
        ...
        if ((this.number == 2) | (this.number == 3)) {
          ...
          if(this.karma > 4){
             ...
             this.niceend = this.add.tileSprite(0, 0, 512, 512, "niceending").setOrigin(0,0).setScale(1.3,1);
             this.letter.destroy();
             return;
          }
        } else {
          ...
          if(this.karma < -4){
            ...
            this.coalend = this.add.tileSprite(0, 0, 512, 512, "coalend").setOrigin(0,0).setScale(1.3);
            this.letter.destroy();
            return;
          }
        }
  ...
  update() {
    ...
    this.nice.tilePositionX -= 2;
    this.naughty.tilePositionX += 2;
